...
import os, sys
import sqlite3  # ADICIONE ESTE IMPORT!

# FUNÇÃO PARA ENCONTRAR ARQUIVOS QUANDO O PROGRAMA ESTIVER COMPILADO
def resource_path(relative_path):
    """ OBTÉM O CAMINHO CORRETO QUANDO RODAR PELO PYINSTALLER """
    try:
        # QUANDO RODAR COMO .EXE
        base_path = sys._MEIPASS
    except Exception:
        # QUANDO RODAR NORMAL NO PYTHON
        base_path = os.path.abspath(".")
    return os.path.join(base_path, relative_path)

# FUNÇÃO PARA O CAMINHO DO BANCO DE DADOS (ADICIONE ESTA!)
def get_db_path():
...
# LOGO
app_img = Image.open(resource_path("imagens/estante-de-livros.png"))
...
def novo_usuario():
    ...
    def add():
        ...
        try:
            conn = sqlite3.connect(get_db_path())  # ← ALTERADO!
            cursor = conn.cursor()
    ...
    #BOTÃO SALVAR
    img_salvar = Image.open(resource_path("imagens/diskette.png")).resize((20, 20))
...
#NOVO EMPRÉSTIMO
#NOVO EMPRÉSTIMO - FUNÇÃO CORRIGIDA
def realizar_emprestimo():
    ...
    def add():
        try:
            #OBTER VALORES DOS CAMPOS
            user_id = entrada_id_usuario.get().strip()
            book_id = entrada_id_livro.get().strip()
    ...
    try:
        ...
        salvar_btn = tk.Button(frameDireita, text="Salvar", command=add,
                             bg="#69069E", fg="white",
                             width=80, height=35, font=('Verdana', 10, 'bold'))
    salvar_btn.grid(row=3, column=0, columnspan=2, pady=20)
...
def ver_livros_emprestados():
    ...
    titulo = tk.Label(frameDireita, text="Todos os Empréstimos",
                      font=('Verdana', 12, 'bold'),
                      bg="#9C09EB")
    titulo.grid(row=0, column=0, columnspan=2, pady=10)

    # OBTÉM DADOS - AGORA USA get_all_loans() EM VEZ DE get_books_on_loan()
    all_loans = get_all_loans() or []
    dados = []

    for loan in all_loans:
        # loan = [id, titulo, nome, sobrenome, data_emprestimo, data_devolucao]
        nome_completo = f"{loan[2]} {loan[3]}"
        data_emprestimo = loan[4] if loan[4] else "N/A"

        # VERIFICA STATUS
        if loan[5]:  # Se tem data_devolucao
            status = f"Devolvido {loan[5]}"
        else:
            status = "Emprestado"

        dado = [loan[0], loan[1], nome_completo, data_emprestimo, status]
        dados.append(dado)

    list_header = ['ID', 'Titulo', 'Usuário', 'Data Empréstimo', 'Status']
    global tree

        # ... (o resto do código da treeview permanece igual)
    ...
    frameDireita.update_idletasks()
    frame_width = frameDireita.winfo_width() - 20
    col_widths = [50, 200, 150, 120, 120]  # AJUSTEI AS LARGURAS
    total_width = sum(col_widths)
    scale = frame_width / total_width
...
#NOVO USUÁRIO
img_user = Image.open(resource_path("imagens/adicionar-usuario.png")).resize((20, 20))
...
#NOVO LIVRO
img_add_book = Image.open(resource_path("imagens/mais.png")).resize((20, 20))
...
#EXIBIR LIVROS
img_book = Image.open(resource_path("imagens/abra-o-livro.png")).resize((20, 20))
...
#EXIBIR USUÁRIOS
img_view_user = Image.open(resource_path("imagens/user.png")).resize((20, 20))
...
#NOVO EMPRÉSTIMO
img_new_emprestimo = Image.open(resource_path("imagens/mais.png")).resize((20, 20))
...
#LIVROS EMPRESTADOS
img_book_emprestado = Image.open(resource_path("imagens/livro.png")).resize((20, 20))
...
#DEVOLVER LIVRO
img_devolver = Image.open(resource_path("imagens/updating.png")).resize((18, 18))
